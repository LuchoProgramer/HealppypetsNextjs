            ...
            {BEFORE_AFTER.map((item, i) => (
              <div key={i} className="bg-white border-2 border-gray-200 rounded-xl p-6 text-center hover:border-[#F2C2EA] transition-all hover:shadow-lg">
                ...
                <div className="flex items-center justify-center gap-3 mb-4">
                  ...
                  <span className="text-3xl font-black text-[#F2C2EA]">{item.after}</span>
                </div>
                <p className="text-sm text-green-600 font-bold">AHORRAS {Number(item.before.slice(1)) - Number(item.after.slice(1))} DÓLARES</p>
              </div>
            ))}
          </div>

          <div className="bg-yellow-50 border-2 border-yellow-200 rounded-xl p-6 text-center max-w-2xl mx-auto">
            <p className="text-yellow-900 font-semibold text-lg">⭐ Además: Al agendar, ¡recibirás recomendaciones personalizadas para tu mascota!</p>
          </div>
        </section>

        {/* Por qué elegir Healppypets */}
        <section className="py-16 border-t border-gray-200">
          <h2 className="text-3xl sm:text-4xl font-bold text-gray-900 mb-12 text-center">¿Por Qué Elegir Healppypets?</h2>

          <div className="grid md:grid-cols-3 gap-8">
            {[
              { emoji: "👩‍⚕️", title: "Carla - Veterinaria Experta", desc: "3+ años de experiencia veterinaria y grooming profesional" },
              { emoji: "❤️", title: "Trato Especial", desc: "Cada mascota es tratada como si fuera la nuestra" },
              { emoji: "⚡", title: "Rápido y Eficiente", desc: "Servicios rápidos sin comprometer la calidad" },
              { emoji: "📍", title: "En Carcelén", desc: "Ubicación accesible con estacionamiento disponible" },
              { emoji: "💰", title: "Precios Justos", desc: "Transparentes y competitivos, sin sorpresas" },
              { emoji: "💬", title: "Disponible por WhatsApp", desc: "Respuestas rápidas a todas tus preguntas" }
            ].map((item, i) => (
          ...
          <div className="max-w-2xl mx-auto space-y-4">
            {FAQ_ITEMS.map((item, i) => (
              <div key={i} className="border border-gray-200 rounded-xl overflow-hidden">
                <button onClick={() => setExpandedFaq(expandedFaq === i ? null : i)} className="w-full p-6 text-left font-semibold text-gray-900 hover:bg-[#F2DFED]/50 transition-colors flex justify-between items-center">
                  {item.q}
                  <span className={`transition-transform ${expandedFaq === i ? "rotate-180" : ""}`}>▼</span>
                </button>
                {expandedFaq === i && (<div className="px-6 pb-6 text-gray-700 border-t border-gray-200">{item.a}</div>)}
              </div>
            ))}
          </div>
        </section>
